...
CVar<RenderDetail_t> r_GeometryDetail(
	"r.GeometryDetail",
	Detail_Medium,
	Cvar_Save,
	"Sets the engine's geometry and shaping quality.",
	CVG_RENDERER
);
CVar<RenderDetail_t> r_TextureDetail(
	"r.TextureDetail",
	Detail_Medium,
	Cvar_Save,
	"Sets the engine's texture quality.",
	CVG_RENDERER
);
...
/*
void *IRenderContext::VirtualPoolAllocate( size_t n )
{
	void *pMemory = g_pResourceAllocator->Alloc( n );
	return pMemory;
}

void *IRenderContext::VirtualPoolClearedAllocate( size_t nElems, size_t nSize )
{
	return memset( g_pResourceAllocator->Alloc( nElems * nSize ), 0, nElems * nSize );
}

void *IRenderContext::VirtualPoolReallocate( void *pOriginalPointer, size_t nSize )
{
	return g_pResourceAllocator->Realloc( pOriginalPointer, nSize );
}

void IRenderContext::VirtualPoolFree( void *pMemory )
{
	g_pResourceAllocator->Free( pMemory );
}
*/
...
#if !defined(SIRENGINE_BUILD_RENDERLIB_GLFW3)
	if ( m_pWindow != NULL ) {
		SIRENGINE_LOG_LEVEL( RenderBackend, ELogLevel::Info, "Destroying SDL2 window handle (0x%lx).", (uintptr_t)m_pWindow );

...
void GLBuffer::Copy( const IRenderBuffer& other )
{
	...
	GL_CALL( nglBindBuffer( GL_COPY_WRITE_BUFFER, m_hBufferID ) );
	GL_CALL( nglBindBuffer( GL_COPY_READ_BUFFER, pBuffer->GetGLObject() ) );
	GL_CALL( nglCopyBufferSubData( GL_COPY_READ_BUFFER, GL_COPY_WRITE_BUFFER, 0, 0, pBuffer->GetSize() ) );
	GL_CALL( nglBindBuffer( GL_COPY_READ_BUFFER, 0 ) );
	GL_CALL( nglBindBuffer( GL_COPY_WRITE_BUFFER, 0 ) );
...
void GLBuffer::Init( const void *pBuffer, uint64_t nSize )
{
	...
	GL_CALL( nglCreateBuffers( 1, &m_hBufferID ) );
	GL_CALL( nglBindBuffer( m_nBufferTarget, m_hBufferID ) );

	if ( r_UseMappedBufferObjects.GetValue() ) {
		if ( m_nUsage == BufferUsage_Constant ) {
			GL_CALL( nglBufferStorage( m_nBufferTarget, nSize, pBuffer, GL_MAP_WRITE_BIT ) );
			void *pMemory = nglMapBufferRange( m_nBufferTarget, 0, nSize, GL_MAP_WRITE_BIT );
			if ( pMemory ) {
				memcpy( pMemory, pBuffer, nSize );
			}
		} else {
			GL_CALL( nglBufferStorage( m_nBufferTarget, nSize, pBuffer, GL_MAP_WRITE_BIT | GL_MAP_COHERENT_BIT | GL_MAP_PERSISTENT_BIT ) );
		...
		GL_CALL( nglBufferData( m_nBufferTarget, nSize, pBuffer, m_nGLUsage ) );
	}

	GL_CALL( nglBindBuffer( m_nBufferTarget, 0 ) );
}
...
void GLBuffer::Shutdown( void )
{
	if ( m_nUsage == BufferUsage_Stream ) {
		GL_CALL( nglBindBuffer( m_nBufferTarget, m_hBufferID ) );
		GL_CALL( nglUnmapBuffer( m_nBufferTarget ) );
		GL_CALL( nglBindBuffer( m_nBufferTarget, 0 ) );
	}
	GL_CALL( nglDeleteBuffers( 1, &m_hBufferID ) );
}
...
void GLBuffer::Clear( void ) {
	...
	GL_CALL( nglBufferData( m_nBufferTarget, m_nBufferSize, NULL, m_nGLUsage ) );
}

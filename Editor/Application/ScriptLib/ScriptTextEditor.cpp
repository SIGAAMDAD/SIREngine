...
void CTextEditor::SetLanguageDefinition(const LanguageDefinition & aLanguageDef)
{
	...
	for (auto& r : mLanguageDefinition.mTokenRegexStrings)
		mRegexList.push_back(eastl::make_pair(std::regex(r.first.c_str(), std::regex_constants::optimize), r.second));
...
CString CTextEditor::GetText(const Coordinates & aStart, const Coordinates & aEnd) const
{
	CString result;
...
CString CTextEditor::GetWordUnderCursor() const
{
	auto c = GetCursorPosition();
	return GetWordAt(c);
}

CString CTextEditor::GetWordAt(const Coordinates & aCoords) const
{
...
void CTextEditor::SetText(const CString & aText)
{
	mLines.clear();
...
void CTextEditor::SetTextLines(const eastl::vector<CString> & aLines)
{
	mLines.clear();
...
void CTextEditor::InsertText(const CString & aValue)
{
...
void CTextEditor::CTextEditor::MoveBottom(bool aSelect)
{
...
CString CTextEditor::GetText() const
{
	return GetText(Coordinates(), Coordinates((int)mLines.size(), 0));
}

eastl::vector<CString> CTextEditor::GetTextLines() const
{
	eastl::vector<CString> result;
...
CString CTextEditor::GetSelectedText() const
{
	return GetText(mState.mSelectionStart, mState.mSelectionEnd);
}

CString CTextEditor::GetCurrentLineText()const
{
...
const CTextEditor::LanguageDefinition& CTextEditor::LanguageDefinition::CPlusPlus()
{
	...
	if (!inited)
	{
		...
		static const char* const identifiers[] = {
			...
			"eastl", "string", "vector", "map", "unordered_map", "set", "unordered_set", "min", "max"
		};
		for (auto& k : identifiers)
		{
			Identifier id;
			id.mDeclaration = "Built-in function";
			langDef.mIdentifiers.insert(eastl::make_pair(CString(k), id));
		}
...
const CTextEditor::LanguageDefinition& CTextEditor::LanguageDefinition::HLSL()
{
	...
	if (!inited)
	{
		...
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("[ \\t]*#[ \\t]*[a-zA-Z_]+", PaletteIndex::Preprocessor));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("L?\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("\\'\\\\?[^\\']\\'", PaletteIndex::CharLiteral));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("[+-]?[0-9]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("0[0-7]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("0[xX][0-9a-fA-F]+[uU]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("[a-zA-Z_][a-zA-Z0-9_]*", PaletteIndex::Identifier));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("[\\[\\]\\{\\}\\!\\%\\^\\&\\*\\(\\)\\-\\+\\=\\~\\|\\<\\>\\?\\/\\;\\,\\.]", PaletteIndex::Punctuation));
...
const CTextEditor::LanguageDefinition& CTextEditor::LanguageDefinition::AngelScript()
{
	...
	if (!inited)
	{
		...
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("L?\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("\\'\\\\?[^\\']\\'", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("[+-]?[0-9]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("0[0-7]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<CString, PaletteIndex>("0[xX][0-9a-fA-F]+[uU]?[lL]?[lL]?", PaletteIndex::Number));
...
void CTextEditorManager::AddScript( CScriptObject *pObject )
{
	ScriptTabData editor;

	editor.pData = pObject;
	editor.bOpened = true;
	SIREngine_strncpyz( editor.szName, pObject->GetName().c_str(), sizeof( editor.szName ) );
	SIREngine_strncpyz( editor.szDisplayName, pObject->GetDisplayName(), sizeof( editor.szDisplayName ) );

	m_Editors.AddItem( editor );
}

void CTextEditorManager::Update( void )
{
	for ( auto& it : m_Editors.GetItems() ) {
		SIREngine_strncpyz( it.second.szName, it.second.pData->GetName().c_str(), sizeof( it.second.szName ) );
		SIREngine_strncpyz( it.second.szDisplayName, it.second.pData->GetDisplayName(), sizeof( it.second.szDisplayName ) );
	}
}

void CTextEditorManager::Draw( void )
{
	...
	ScriptTabData *pCurrent = m_Editors.GetCurrent();
	if ( pCurrent ) {
		CTextEditor *pEditor = pCurrent->pData->GetEditor();

		pEditor->Render( pCurrent->szDisplayName );
		if ( pCurrent->pData->GetEditor()->IsTextChanged() && pCurrent->szDisplayName[ strlen( pCurrent->szDisplayName ) - 1 ] != '*' ) {
			strncat( pCurrent->szDisplayName, "*", sizeof( pCurrent->szDisplayName ) - 1  );
			CProjectManager::Get()->GetProject()->SetModified( true );
		}

		// editor shortcuts
		if ( ImGui::IsKeyDown( ImGuiKey_UpArrow ) && pEditor->GetCursorPosition().mLine == 0 ) {
			pEditor->SetCursorPosition( CTextEditor::Coordinates( 0, 0 ) );
		}
		if ( ImGui::IsKeyDown( ImGuiKey_DownArrow ) && pEditor->GetCursorPosition().mLine == pEditor->mLines.size() - 1 ) {
			pEditor->SetCursorPosition( CTextEditor::Coordinates( pEditor->mLines.size() - 1, pEditor->GetLineMaxColumn(
				pEditor->mLines.size() - 1
			) ) ) ;
		}
	}
	ImGui::End();
...
{
	CEditorApplication::Get().AddWidget( &g_TextManager );
}
